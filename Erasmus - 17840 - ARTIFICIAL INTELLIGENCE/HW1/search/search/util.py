...
class PriorityQueueWithPathFunction(PriorityQueue):
    ...
    Queue and the Stack classes. This is designed for drop-in replacement for
    those two classes. The caller has to provide a priority function, which
    extracts each item's priority using it's path and/or heuristic.

    Design is similar to PriorityQueueWithFunction, however, we utilise the
    the Cost function on the Path variable and if needed the heuristic of
    the State.

    If heuristic is not provided, works as a priority queue for for UCS.
    Else (heuristic is provided), works as a priority queue for A* search.
    
    """
    def  __init__(self, priorityFunction, problem = None, heuristic = None):
        "priorityFunction (item) -> priority"
        self.priorityFunction = priorityFunction        # store the priority function
    ...
    def push(self, item):
        ...
        if(self.heuristic is not None):
            PriorityQueue.push(self, item, self.priorityFunction(item[1]) + self.heuristic(item[0],self.problem))
        else:
