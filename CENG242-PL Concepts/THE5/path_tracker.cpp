...
PathTracker::PathTracker(int *existing_path, int number_of_vectors)
{
  ...
  for (int i=0;i < number_of_vectors*2 ; i=i+2){
    distance = distance + calculate_l2(final_x + existing_path[i],final_y + existing_path[i+1],final_x,final_y);
    final_x  = final_x + existing_path[i];
    final_y = final_y + existing_path[i+1];
...
{
  distance = distance + calculate_l2(final_x + new_vector[0],final_y + new_vector[1],final_x,final_y);
  this->final_x = final_x + new_vector[0];
  this->final_y = final_y + new_vector[1];
  displacement = calculate_displacement();
  return *this;
}
...
{ 
  if (this->displacement == rhs.displacement){ 
    return true; }
  else { return false;}
}
...
{
  distance = calculate_l2(final_x,final_y,x,y);
}
